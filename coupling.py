...
def source_coupling(k0, h0, length, slope, z_interval, dx):
    ...
    Change_CB = []
    for i in range(iterations):
        h_low = h0 + z_interval*i*slope
        h_high = h0 + z_interval*(i+1)*slope
...
if __name__ == '__main__':
    ...
    AB, As, Bs = source_coupling(k0, h0, 1000, slope, z_interval, dx)
    fig, ax = plt.subplots()
    ax.plot(range(len(Bs)), Bs, 'k--', label='A')
    ax.set_ylabel('B')

    ax2 = ax.twinx()
    ax2.plot(range(len(As)), As, 'k-', label='B')
    ax2.set_ylabel('A')
