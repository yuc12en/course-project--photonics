...
top_width = 20-h0/2
bottom_width = 3-h0/2
ts = np.zeros(len(slope), dtype='complex')

for i in range(len(slope)):
    x, E, out = get_inverse_taper_pattern(k0, h0, bottom_width, top_width, slope=slope[i] ,n_interval=int(h0*slope[i]*2), n_step=5, dx=0.1, show='none')
    source = np.exp(-(x/5)**2)
    source = normalized_distribution(x, source, w, mu, k0*nf)
    I = simpson(source*np.abs(E), x=x)
